...
from generateDOE_AdditionalCity import generate_designs_and_cities
...
def run_simulation(AirshipDesigns,numberOfNewCities = 1):
    print(datetime.now())
    ...
    for dataDOE in AirshipDesigns:
        ...
        cityCoordinates = [ [-3.196, -59.826],  # Careiro
                            ...
                            [dataDOE.item(8), dataDOE.item(9)]]  #additional city
        lat1 = hubCoordinates[0] * np.pi / 180
        lon1 = hubCoordinates[1] * np.pi / 180
        lat2 = dataDOE.item(8) * np.pi / 180
        lon2 = dataDOE.item(9) * np.pi / 180
        # distance in nm
        newCityDistanceToHub = 0.539957 * 6371 * 2 * np.arcsin( np.sqrt( (np.sin((lat1-lat2)/2))**2 + np.cos(lat1)*np.cos(lat2)*(np.sin((lon1-lon2)/2))**2 ))
        newCityDistanceToHubViaBoat = 1.97 * newCityDistanceToHub
        LoadingResources = 1
        boatCount = np.array([5., 7., 7., 12.]) # [5.+5., 7.+28., 7.+4., 12.+7.] # with manaus boats divided between
        CityToHubBoatDistance =  np.array([20.,38.,47.,60., newCityDistanceToHubViaBoat]) #nautical miles
    ...
    outputImpacts = pd.DataFrame(
        {
            "Lat":AirshipDesigns[:,8],
            "Lon":AirshipDesigns[:,9],
...
## TESTS
# # Main
AirshipDesigns = generate_designs_and_cities(payloadrange=16, speedrange=37, threshrange=1, loadraterange=0.2,
                fleetrange=[1,2], latRange=[-3.117, -3.6], lonRange=[-60.72,-59.6], setLatSteps = 25, setLonSteps=25)
